...
if uploaded_files:
    for file in uploaded_files:
        ...
        if isinstance(df, pd.DataFrame):
            ...
            column_to_sort = st.selectbox("Choose column to sort by", df.columns)
            sort_order = st.radio("Sort order", ["Ascending", "Descending"])

            if st.button(f"Sort by {column_to_sort}"):
                if sort_order == "Ascending":
                    df = df.sort_values(by=column_to_sort, ascending=True)
                else:
                    df = df.sort_values(by=column_to_sort, ascending=False)
                st.write(f"Data sorted by {column_to_sort} in {sort_order} order!")
